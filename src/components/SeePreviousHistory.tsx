...
import { Check, List, X, Calendar, Users, FileText, Trophy, Plus, Minus, Edit, ChevronDown, ChevronRight } from "lucide-react";
...
export const SeePreviousHistory = ({
  ...
  const calculateDifferences = (currentSave: SavesResponse, previousSave: SavesResponse | null) => {
    ...
    const differences = {
      ...
      validationsAdded: [] as { player: string; proposition: string; description: string }[],
    ...
    const previousPropositionTexts = previousSave.data.propositions.map(p => p.text);

    differences.propositionsAdded = currentPropositionTexts.filter(text => !previousPropositionTexts.includes(text));
    differences.propositionsRemoved = previousPropositionTexts.filter(text => !currentPropositionTexts.includes(text));
  ...
  const renderDifferences = (differences: any) => {
    ...
    return (
      <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200 space-y-2">
        <h4 className="text-sm font-semibold text-blue-800 mb-2">Changements depuis la sauvegarde précédente :</h4>
        
        {differences.playersAdded.length > 0 && (
          <div className="flex items-start gap-2">
            <Plus size={14} className="text-success-600 mt-0.5 flex-shrink-0" />
            <div className="text-xs text-success-700">
              <span className="font-medium">Joueurs ajoutés :</span> {differences.playersAdded.join(", ")}
            </div>
          </div>
        )}
        ...
        {differences.validationsAdded.length > 0 && (
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-success-700">
              ...
              <div className="ml-2 space-y-1">
                {differences.validationsAdded.map((validation: any, index: number) => (
                  <div key={index} className="bg-success-100 p-2 rounded text-xs">
                    <div className="font-medium">{validation.player} → {validation.proposition}</div>
                    <div className="text-success-600 italic">{validation.description}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {differences.validationsRemoved.length > 0 && (
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-error-700">
              ...
              <div className="ml-2 space-y-1">
                {differences.validationsRemoved.map((validation: any, index: number) => (
                  <div key={index} className="bg-error-100 p-2 rounded text-xs">
                    <div className="font-medium">{validation.player} → {validation.proposition}</div>
                  </div>
                ))}
              </div>
        ...
        {differences.propositionsAdded.length > 0 && (
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-primary-700">
              <span className="font-medium">Propositions ajoutées :</span> {differences.propositionsAdded.join(", ")}
            </div>
          </div>
        )}
        ...
        {differences.propositionsRemoved.length > 0 && (
          <div className="flex items-start gap-2">
            <Minus size={14} className="text-error-600 mt-0.5 flex-shrink-0" />
            <div className="text-xs text-error-700">
              <span className="font-medium">Propositions supprimées :</span> {differences.propositionsRemoved.join(", ")}
            </div>
          </div>
        )}
      </div>
          ...
          <div className="overflow-y-auto max-h-[60vh] space-y-4">
            {saves.length === 0 ? (
              ...
              saves.map((save, index) => {
                ...
                const previousSave = index < saves.length - 1 ? saves[index + 1] : null;
                ...
                return (
                  ...
                  >
                    ...
                    {differences && isExpanded && renderDifferences(differences)}
